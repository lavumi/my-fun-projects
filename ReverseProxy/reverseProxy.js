...
function ReverseProxy(opts) {
    ...
    if (opts.ssl) {
        if (_.isArray(opts.ssl)) {
            opts.ssl.forEach(function (sslOpts) {
                _this.setupHttpsProxy(proxy, websocketsUpgrade, sslOpts);
            })
        } else {
    ...
    // Plain HTTP Proxy
    //
    var server = this.setupHttpProxy(proxy, websocketsUpgrade, opts);
    ...
    console.log('Started a Redbird reverse proxy server on port %s', opts.port);


    function websocketsUpgrade(req, socket, head) {
        socket.on('error', function (err) {
            console.log(err, 'WebSockets error');
        });
        var src = _this._getSource(req);
        _this._getTarget(src, req).then(function (target) {
            console.log({ headers: req.headers, target: target }, 'upgrade to websockets');
            if (target) {
...
ReverseProxy.prototype.setupHttpProxy = function (proxy, websocketsUpgrade, opts) {
    var _this = this;
    ...
    var server = this.server = httpServerModule.createServer(function (req, res) {
        ...
        _this._getTarget(src, req).bind(_this).then(function (target) {
            if (target) {
                console.log(" setupHttpProxy getTargt");
...
function shouldRedirectToHttps(certs, src, target, proxy) {
    return target.redirectPort || (certs && src in certs && target.sslRedirect);
}

ReverseProxy.prototype.setupHttpsProxy = function (proxy, websocketsUpgrade,  sslOpts) {
    var _this = this;
    var https;
    ...
    httpsServer.on('error', function (err) {
        console.log(err, 'HTTPS Server Error');
    });

    httpsServer.on('clientError', function (err) {
        console.log(err, 'HTTPS Client  Error');
    });

    console.log('Listening to HTTPS requests on port %s', sslOpts.port);
...
ReverseProxy.buildTarget = function (target, opts) {
    ...
    target.redirectPort = opts.ssl && opts.ssl.redirectPort;
    console.log("set ridirect port : ", target.redirectPort);
    return target;
};
...
ReverseProxy.prototype.register = function (src, target, opts) {
    ...
    console.log(JSON.stringify({ from: src, to: target }), 'Registered a new route');
    return this;
};
...
ReverseProxy.prototype.unregister = function (src, target) {
    ...
    if (i < routes.length) {
        ...
        console.log(JSON.stringify({ from: src, to: target }), 'Unregistered a route');
    }
    return this;
};
...
ReverseProxy.prototype._getTarget = function (src, req) {
    ...
    return this.resolve(src, url, req).bind(this).then(function (route) {
        if (!route) {
            console.log({ src: src, url: url }, 'no valid route found for given source');
            return;
        }
...
function redirectToHttps(req, res, target, ssl) {
    ...
    var targetPort = target.redirectPort || (ssl.redirectPort || ssl.port);
    var hostname = req.headers.host.split(':')[0] + (targetPort ? ':' + targetPort : '');
    var url = 'https://' + path.join(hostname, req.url);
    console.log('Redirecting %s to %s', path.join(req.headers.host, req.url), url);
    //
